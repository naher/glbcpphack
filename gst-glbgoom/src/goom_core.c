...
PluginInfo * goom_init (guint32 resx, guint32 resy)
{
  ...
  goomInfo->glb_basic_fx = glb_basic_fx_create ();
  goomInfo->glb_basic_fx.init (&goomInfo->glb_basic_fx, goomInfo);
...
guint32 *
glb_goom_update (PluginInfo * goomInfo, gint16 data[2][512], int forceMode,
    float fps) 
{
  Pixel *return_val;
  guint32 pointWidth;
...
*/
  ...
  goomInfo->glb_basic_fx.apply (&goomInfo->glb_basic_fx, goomInfo->p2, return_val = goomInfo->p1, goomInfo);
  //goomInfo->glb_basic_fx.apply (&goomInfo->glb_basic_fx, goomInfo->p1, return_val = goomInfo->p2, goomInfo);
...
// ---- start goom's original update code ---------------

guint32 *
original_goom_update (PluginInfo * goomInfo, gint16 data[2][512], int forceMode,
    float fps)
{
  Pixel *return_val;
  guint32 pointWidth;
...
//	goomInfo->p2 = tmp;
  ...
  /* Zoom here ! */
  zoomFilterFastRGB (goomInfo, goomInfo->p2, goomInfo->p1, pzfd,
      goomInfo->screen.width, goomInfo->screen.height,
      goomInfo->update.switchIncr, goomInfo->update.switchMult);
  ...
  goomInfo->glb_basic_fx.apply (&goomInfo->glb_basic_fx, goomInfo->p1, goomInfo->p2,
      goomInfo);
  goomInfo->glb_basic_fx.apply (&goomInfo->glb_basic_fx, goomInfo->p2, goomInfo->p1,
      goomInfo);
...
goom_close (PluginInfo * goomInfo)
{
  ...
  goomInfo->glb_basic_fx.free (&goomInfo->glb_basic_fx);
...
goom_update (PluginInfo * goomInfo, gint16 data[2][512], int forceMode,
    float fps) 
{
	//return original_goom_update (goomInfo, data, forceMode, fps);
	return glb_goom_update (goomInfo, data, forceMode, fps);
}
