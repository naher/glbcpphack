  ...
  while (gst_adapter_available (spectrum->adapter) >= wanted) {
    ...
    if (spectrum->num_frames >=
      ...
      if (spectrum->message) {
		...
		msg.spectrum_bands = spectrum->bands;
		msg.timestamp = endtime;
		for (i = 0; i < spectrum->bands; i++) {
			msg.magnitude[i] = spect_magnitude[i];
		}
		
		writeMessage(&msg);

		/*g_print ("Spectrum: %" GST_TIME_FORMAT "\n",
          GST_TIME_ARGS (blktime));*/
...
{
  return gst_element_register (plugin, "glbspectrum", GST_RANK_NONE,
      GST_TYPE_SPECTRUM);
}
